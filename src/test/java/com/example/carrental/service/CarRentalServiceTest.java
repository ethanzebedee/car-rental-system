...
class CarRentalServiceTest {
    ...
    void reservationIdIsUnique() {
        ...
        when(carRepository.findByType(CarType.SEDAN)).thenReturn(List.of(sedan1, sedan2));
        // Each call returns the reservations saved so far, so the second reserveCar
        // call sees sedan1's reservation and must skip it (conflict) and pick sedan2.
        when(reservationRepository.findByCarIdIn(anyList()))
                .thenAnswer(inv -> new ArrayList<>(savedReservations));
